...
export interface InvoiceUpdate extends Partial<Omit<InvoiceCreate, 'items'>> {
  items?: InvoiceItemCreate[];
}
...
class InvoiceApiService {
  async getInvoices(filters: InvoiceFilters = {}): Promise<InvoiceListResponse> {
    ...
    const url = `/sales/invoices${queryString ? `?${queryString}` : ''}`;
  ...
  async createInvoice(invoiceData: InvoiceCreate): Promise<Invoice> {
    return await apiService.post<Invoice>('/sales/invoices', invoiceData);
  }

  async updateInvoice(id: number, invoiceData: InvoiceUpdate): Promise<Invoice> {
    return await apiService.put<Invoice>(`/sales/invoices/${id}`, invoiceData);
  }

  async deleteInvoice(id: number): Promise<void> {
    try {
      return await apiService.delete<void>(`/sales/invoices/${id}`);
    } catch (error: any) {
  ...
  async getInvoiceSummary(): Promise<InvoiceSummary[]> {
    return await apiService.get<InvoiceSummary[]>('/sales/invoices/summary/list');
  }

  async getGSTSlabs(): Promise<GSTSlab[]> {
    return await apiService.get<GSTSlab[]>('/sales/invoices/gst-slabs');
  }

  async createGSTSlab(gstSlabData: GSTSlabCreate): Promise<GSTSlab> {
    return await apiService.post<GSTSlab>('/sales/invoices/gst-slabs', gstSlabData);
  }

  async seedGSTSlabs(): Promise<{ message: string }> {
    return await apiService.post<{ message: string }>('/sales/invoices/seed-gst-slabs');
  }

  async sendWhatsApp(request: WhatsAppInvoiceRequest): Promise<WhatsAppResponse> {
    return await apiService.post<WhatsAppResponse>('/sales/invoices/send-whatsapp', request);
  }
}
