# PowerShell script to update PostgreSQL tables to use composite primary keys
...
Write-Host "Updating PostgreSQL tables to use composite primary keys..." -ForegroundColor Green

# SQL commands to update tables with composite primary keys
$sql = @"
...
DROP TABLE IF EXISTS gst_slabs CASCADE;

-- Recreate tables with composite primary keys

-- Create users table (account_id, id as composite PK)
CREATE TABLE users (
...
CREATE TABLE invoice_items (
    ...
    PRIMARY KEY (account_id, id),
    FOREIGN KEY (invoice_account_id, invoice_id) REFERENCES invoices(account_id, id) ON DELETE CASCADE,
    FOREIGN KEY (item_account_id, item_id) REFERENCES items(account_id, id)
);
...
CREATE TABLE payments (
    ...
    FOREIGN KEY (customer_account_id, customer_id) REFERENCES customers(account_id, id),
    FOREIGN KEY (invoice_account_id, invoice_id) REFERENCES invoices(account_id, id)
);
...
CREATE TABLE purchases (
    ...
    FOREIGN KEY (vendor_account_id, vendor_id) REFERENCES vendors(account_id, id)
);
...
CREATE TABLE purchase_items (
    ...
    amount DECIMAL(12, 2) NOT NULL,
    PRIMARY KEY (account_id, id),
    FOREIGN KEY (purchase_account_id, purchase_id) REFERENCES purchases(account_id, id) ON DELETE CASCADE,
    FOREIGN KEY (item_account_id, item_id) REFERENCES items(account_id, id)
);
...
CREATE TABLE credit_payments (
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (account_id, id),
    FOREIGN KEY (credit_account_id, credit_id) REFERENCES credits(account_id, id) ON DELETE CASCADE
);
...
CREATE TABLE sales_return_items (
    ...
    amount DECIMAL(12, 2) NOT NULL,
    PRIMARY KEY (account_id, id),
    FOREIGN KEY (return_account_id, return_id) REFERENCES sales_returns(account_id, id) ON DELETE CASCADE,
    FOREIGN KEY (item_account_id, item_id) REFERENCES items(account_id, id)
);
...
CREATE TABLE shipment_items (
    ...
    quantity DECIMAL(10, 2) NOT NULL,
    PRIMARY KEY (account_id, id),
    FOREIGN KEY (shipment_account_id, shipment_id) REFERENCES shipments(account_id, id) ON DELETE CASCADE,
    FOREIGN KEY (item_account_id, item_id) REFERENCES items(account_id, id)
);
...
try {
    Write-Host "Executing SQL commands to update table structure..." -ForegroundColor Yellow
